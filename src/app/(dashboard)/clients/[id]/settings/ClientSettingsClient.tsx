...
import {
  ...
  Trash2, Clock, Pencil, Save, Code2,
} from 'lucide-react';
...
export default function ClientSettingsClient({ client, initialDomains, workspaceId, appHostname, appARecord, appUrl, canManage }: Props) {
  ...
  function getAddPreview(): string { return buildFullDomain(addBaseDomain, addMode, addSubdomain); }
  function getEditPreview(): string { return buildFullDomain(editBaseDomain, editMode, editSubdomain); }
  ...
  function getBaseDomain(domain: string): string { return domain.split('.').slice(-2).join('.'); }
