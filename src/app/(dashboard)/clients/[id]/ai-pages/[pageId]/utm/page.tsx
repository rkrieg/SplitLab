...
import UTMPickerClient, { type StoredFieldSelectors } from './UTMPickerClient';

interface PageProps {
  params: { id: string; pageId: string };
}

export default async function UTMPickerPage({ params }: PageProps) {
  ...
  const { data: page } = await db
    .from('pages')
    .select('id, name, slug, schema_json, field_selectors_json, is_published, published_url')
    .eq('id', params.pageId)
