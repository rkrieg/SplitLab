...
export default function AnalyticsClient({ test: initialTest, appUrl, clientId, clientName, domain }: Props) {
  ...
  return (
    <div>
      ...
      {/* ═══ TEST URL BAR ═══ */}
      {serveUrl ? (
        <div className="border-b border-slate-200 dark:border-slate-800 px-6 py-2.5 bg-slate-50 dark:bg-slate-800/50 flex items-center gap-3">
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide flex-shrink-0">Test URL</span>
          <code className="flex-1 text-xs text-indigo-500 dark:text-indigo-400 font-mono truncate min-w-0">{serveUrl}</code>
          <button
            onClick={copyServeUrl}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:border-indigo-400 hover:text-indigo-500 transition-colors flex-shrink-0"
          >
            {copiedServe ? <Check size={11} className="text-green-500" /> : <Copy size={11} />}
            {copiedServe ? 'Copied!' : 'Copy'}
          </button>
          <a
            href={serveUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-indigo-500 text-white hover:bg-indigo-600 transition-colors flex-shrink-0"
          >
