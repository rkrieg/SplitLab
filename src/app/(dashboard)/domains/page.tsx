...
export default async function AllDomainsPage() {
  ...
  // Admins see all domains; others see only domains for their workspaces
    ...
    const { data } = await db
      ...
      .order('created_at', { ascending: false }) as unknown as {
        data: Array<{
          ...
          verified_at: string | null; created_at: string; workspace_id: string;
              ...
              {domains.map(d => (
                <div key={d.id} className="card p-5 flex items-center gap-4 hover:border-slate-300 dark:hover:border-slate-600 transition-colors">
                  <div className="flex-1 min-w-0">
                    ...
                    {d.client_name && (
                      <p className="text-xs text-slate-400 dark:text-slate-500 ml-[23px]">Client: {d.client_name}</p>
                    )}
                  </div>
                  {d.client_id && (
                    <Link
                    ...
                    >
                      {d.verified ? 'Manage' : 'Configure DNS'}
                    </Link>
                  )}
                </div>
              ))}
